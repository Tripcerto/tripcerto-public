...
This repo is tripcerto.com: the public site for Tripcerto's two products, Engage and Workspace. Five pages (home, Engage, Workspace, Pilot, Trust) and two static legal pages, built with Vite and deployed by Vercel from `main`. It is separate from the product monorepo on purpose: nothing here runs the product.
...
- The privacy notice and terms as static HTML under `public/legal/`.
...
```
index.html, engage/, workspace/, pilot/, trust/   # one entry per page, each with its own head tags
src/
  boot.tsx                 # mounts a page
  main.tsx, engage.tsx, workspace.tsx, pilot.tsx, trust.tsx   # entry files
  App.tsx                  # the home page; the others are in pages/
  pages/                   # EngagePage, WorkspacePage, PilotPage, TrustPage
  content/                 # every string on the site, one file per page, keyed by reference
  components/site/         # Nav, Hero, PageHero, EngageSection, WorkspaceSection, Opportunity, Audience, Close, Footer, Section, Rows, Stage, Band, Reveal, Wordmark
  ...
  site.test.tsx, test/     # the four checks run over every page, and the jsdom setup
public/
...
npm run build      # tsc -b && vite build; emits all five pages into dist/
npm run lint       # eslint .
npm run test:run   # vitest: the same four checks over every page, plus the brand kit's
