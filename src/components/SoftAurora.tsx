...
float auroraGlow(float t, vec2 shift) {
  ...
  return 0.3 * max(exp(uBandSpread * (1.0 - 0.7 * abs(noiseVal + yBand))), 0.0);
}
...
void main() {
  ...
  // Luminance modulation only — phases locked across RGB so the tint stays gold
  float lum1 = 0.5 + 0.5 * cos(TAU * (uv.x + uTime * uSpeed * 0.2 * uColorSpeed));
  float lum2 = 0.5 + 0.5 * cos(TAU * (uv.x + uTime * uSpeed * 0.1 * uColorSpeed) + TAU * 0.5);

  vec3 col = vec3(0.0);
  col += 0.99 * auroraGlow(t, shift) * lum1 * uColor1;
  col += 0.99 * auroraGlow(t + uLayerOffset, shift) * lum2 * uColor2;
