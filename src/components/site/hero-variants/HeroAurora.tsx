...
/* ogl leaves the main bundle; the copy paints first and the band follows. */
const Aurora = lazy(() => import('@/components/ui/aurora').then((m) => ({ default: m.Aurora })))

/* Pink to peach, left to right: the same run as the band gradient. Module-level
   so the shader is built once; the effect re-runs on any new reference. */
const AURORA_STOPS: [string, string, string] = ['#E8437E', '#FF5C6C', '#FF9B7A']
