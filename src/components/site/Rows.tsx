...
/* Ruled rows read from the top: the subject and a short note under it on
   ...
   the link. */
export function Rows({ rows, className }: { rows: ReadonlyArray<Row>; className?: string }) {
  return (
    <ul role="list" className={cn('mt-12 divide-y divide-line md:mt-14', className)}>
      {rows.map(({ name, note, line, href }) => (
        <li key={name} className="grid grid-cols-1 gap-2 py-6 md:grid-cols-[minmax(0,2fr)_minmax(0,3fr)] md:gap-x-12 md:gap-y-0">
          <div>
            <h3 className="text-[17px] leading-[1.4] font-semibold">
              {href ? (
            ...
            <p className="mt-1 text-[14px] leading-[1.5] text-dim">{note}</p>
          </div>
