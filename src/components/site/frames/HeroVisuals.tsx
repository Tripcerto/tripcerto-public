...
/* The two product frames as one composition: the Workspace window sits up
   ...
   below the window and lapping only its edge, so Stella's pane in the
   window stays readable. On small screens the pair is a picture, so the
   phone laps further in. Both are sized from the column width, so the pair
...
export function HeroVisuals({ window, phone }: { window: ReactNode; phone: ReactNode }) {
  return (
    <div className="relative mx-auto flow-root w-full max-w-[560px] lg:max-w-none">
      <div className="animate-pop mb-[20%] ml-[18%] sm:mb-[16%] lg:mb-[12%] lg:ml-[29%]" style={delay(0.1)}>
        {window}
      </div>
      <div className="animate-pop absolute bottom-0 left-0 z-10 w-[42%] sm:w-[38%] lg:w-[31%]" style={delay(0.3)}>
