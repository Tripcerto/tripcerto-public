...
/* The two product frames as one composition: the Workspace window sits up
   ...
   loaded on the phone. On large screens the pair sits centred in its half
   of the hero, clear of the copy. Both are sized from the column width, so the pair
   keeps its shape from a phone screen to a 1680 shell. Both rise together;
   the phone then floats. */
export function HeroVisuals({ window, phone }: { window: ReactNode; phone: ReactNode }) {
  return (
    <div className="relative mx-auto flow-root w-full max-w-[560px] lg:w-[90%] lg:max-w-none">
